...
using TimeWindowTweaks.UIPatches;
using UIFramework.Transform;
using UnityModManagerNet;

namespace TimeWindowTweaks
{
    [PublicAPI]
    public static class Main
    {
        private const string PluginIdentifier = "CzBuCHi.TimeWindowTweaks";
        ...
        private static bool OnToggle(UnityModManager.ModEntry modEntry, bool value /* to active or deactivate */) {
            if (value) {
                UIPanelTransformRegistry.RegisterTransformer<TimeWindowTransformer>();

                var harmony = new HarmonyLib.Harmony(PluginIdentifier);
                harmony.PatchAll();
            } else {
                UIPanelTransformRegistry.UnregisterTransformer<TimeWindowTransformer>();
